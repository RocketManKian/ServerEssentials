...
public class HideBalance implements CommandExecutor {
            ...
            if (ServerEssentials.permissionChecker(player, "se.hidebalance")) {
                if (args.length == 0) {
                    if (!UserFile.fileConfig.getBoolean(player.getUniqueId() + ".balancehidden")) {
                        ...
                        String msg = Lang.fileConfig.getString("hidebalance-toggle-enabled");
                        ...
                        String msg = Lang.fileConfig.getString("hidebalance-toggle-disabled");
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', hex(msg)));
                        return true;
                    }
                } else {
                    String msg = Lang.fileConfig.getString("incorrect-format").replace("<command>", "/hidebalance");
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', hex(msg)));
                    return true;
                }
            }
